/**
 * Agent 客户端（统一渠道版）
 * channel-qiwei 将企微消息归一化后，POST 到 server 的 /api/channels/incoming
 * server 异步处理后，通过 POST /api/qiwei/send 回调发送回复
 */

import { qiweiConfig } from "../config";

const AGENT_SERVER_URL = qiweiConfig.agentServerUrl;

/**
 * 统一入站消息格式（与 server 的 IncomingMessage 一致）
 * messageType 支持所有用户消息类型：text/image/voice/video/file/sticker 等
 */
