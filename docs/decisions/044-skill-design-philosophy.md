...
### 解决方案：Always + On-Demand

Agent 绑定的每个 skill 有两种加载模式（per-agent 配置）：
...
│                                                   │
│  Always Skills（必召回）                            │
│  ├── name + description + 完整 readme → 内联到      │
│  │   SystemPrompt 的 {{skills}} 展开结果中           │
│  ├── tools → 注册为可调用工具                        │
...
Agent 配置中的 `skills` 字段是结构化绑定列表（`[{id, mode}]`），每个绑定指定加载模式：

- `mode: "always"` → skill 完整内容内联到 SystemPrompt，tools 注册
- `mode: "on_demand"` → 仅索引行出现在 SystemPrompt，通过 `load_skill` 按需获取
- `skills` 为空 → 不绑定任何 skill
- `load_skill` 可以访问所有 enabled skill，不受绑定限制

同一个 skill 在不同 agent 上可以有不同的加载模式。
